...
#define MT7621_SPI_DEFAULT_CLK_DIV	7
...
static int mt7621_spi_read(struct mt7621_spi *ms, u8 *buf, size_t len)
{
	...
	while (len) {
		rx_len = min(len, (size_t) MT7621_RX_FIFO_LEN);
...
static int mt7621_spi_write(struct mt7621_spi *ms, const u8 *buf, size_t len)
{
	...
	while (len) {
		tx_len = min(len, (size_t) MT7621_TX_FIFO_LEN);

		opcode_len = min(tx_len, (size_t) 4);
...
static int mt7621_spi_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(hclk)) {
		dev_err(&pdev->dev, "unable to get hclk, err = %d\n", status);
		return PTR_ERR(hclk);
	}
